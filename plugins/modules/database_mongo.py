...
"""This module contains all of the functionality for Linode Mongo Databases."""
...
from linode_api4.objects import MongoDatabase
...
import ansible_collections.linode.cloud.plugins.module_utils.doc_fragments.database_mongo as docs
...
specdoc_meta = dict(
    description=[
        'Manage a Linode MongoDB database.'
    ],
    ...
    return_values=dict(
        database=dict(
            ...
            docs_url='https://www.linode.com/docs/api/databases/'
                     '#managed-mongodb-database-view__response-samples',
            type='dict',
        ...
        backups=dict(
            ...
            docs_url='https://www.linode.com/docs/api/databases/'
                     '#managed-mongodb-database-backup-view__responses',
            type='dict',
            sample=docs.result_backups_samples
        ),
        ssl_cert=dict(
            description='The SSL CA certificate for an accessible Managed MongoDB Database.',
            docs_url='https://www.linode.com/docs/api/databases/'
                     '#managed-mongodb-database-ssl-certificate-view__responses',
            type='dict',
            sample=docs.result_ssl_cert_samples
        ),
        credentials=dict(
            description='The root username and password for an accessible Managed MongoDB Database',
            docs_url='https://www.linode.com/docs/api/databases/'
                     '#managed-mongodb-database-credentials-view__responses',
            type='dict',
...
class Module(LinodeModuleBase):
    ...
    def _get_database_by_label(self, label: str) -> Optional[MongoDatabase]:
        try:
            resp = [db for db in self.client.database.mongodb_instances() if db.label == label]
    ...
    @staticmethod
    def _wait_for_database_status(database: MongoDatabase,
    ...
    def _create_database(self) -> Optional[MongoDatabase]:
        params = self.module.params

        create_params = {
            'allow_list', 'cluster_size', 'encrypted', 'compression_type', 'ssl_connection'
        }

        additional_args = {k: v for k, v in params.items() if k in create_params and k is not None}

        try:
            return self.client.database.mongodb_create(
                params.get('label'),
                params.get('region'),
                params.get('engine'),
                params.get('type'),
    ...
    def _update_database(self, database: MongoDatabase) -> None:
        try:
            database._api_get()
    ...
    def _write_result(self, database: MongoDatabase) -> None:
        # Force lazy-loading
        database._api_get()
...
def main() -> None:
    """Constructs and calls the database_mongodb module"""
    Module()
