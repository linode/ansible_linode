...
"""This module allows users to retrieve information about a Linode Mongo Managed Database."""
...
from linode_api4 import MongoDatabase
...
import ansible_collections.linode.cloud.plugins.module_utils.doc_fragments.database_mongodb \
    as docs_parent
import ansible_collections.linode.cloud.plugins.module_utils.doc_fragments.database_mongodb_info \
    as docs
...
spec = dict(
    ...
    id=dict(type='str', description='The ID of the Mongo Database.'),
    label=dict(type='str', description='The label of the Mongo Database.'),
)

specdoc_meta = dict(
    description=[
        'Get info about a Linode Mongo Managed Database.'
    ],
    ...
    return_values=dict(
        ...
        ssl_cert=dict(
            description='The SSL CA certificate for an accessible Managed Mongo Database.',
            docs_url='https://www.linode.com/docs/api/databases/'
        ...
        credentials=dict(
            description='The root username and password for an accessible Managed Mongo Database.',
            docs_url='https://www.linode.com/docs/api/databases/'
                     '#managed-mongodb-database-credenti'
                     'als-view__responses',
            type='dict',
...
class Module(LinodeModuleBase):
    ...
    def _get_database_by_id(self, database_id: int) -> MongoDatabase:
        try:
            database = MongoDatabase(self.client, database_id)
            database._api_get()
            return database
        except Exception as exception:
